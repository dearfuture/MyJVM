...
from rtda.heap.ExceptionTable import ExceptionTable
...
class Method(ClassMember):
    ...
    def copy_attributes(self, cf_method: MemberInfo):
        ...
        if code_attr:
            ...
            self.exception_table = ExceptionTable.new_exception_table(code_attr.exception_table, self.get_class().rt_constant_pool)

            self.line_number_table = code_attr.line_number_table_attributes
    ...
    def find_exception_handler(self, clazz, pc):
        handler = self.exception_table.find_exception_handler(clazz, pc)
